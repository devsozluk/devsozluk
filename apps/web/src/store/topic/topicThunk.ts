...
export const getBySlugTopic = createAsyncThunk(
  "topic/getBySlug",
  async (payload: { slug?: string }, thunkAPI) => {
...
export const getLatestEntries = createAsyncThunk(
  "topic/getLatestEntries",
  async () => {
...
export const createTopic = createAsyncThunk(
  "topic/createTopic",
  async (
    payload: { values: CreateTopicData; formikActions: any },
    thunkAPI
  ) => {
    ...
    if (topic) {
      const entry = await thunkAPI.dispatch(
        createEntry({
          ...payload.values,
          topic: topic._id,
          author: auth.user?._id,
...
const getTopicEntries = createAsyncThunk(
  "topic/getTopicEntries",
  async (payload: { topic: string }, { rejectWithValue }) => {
...
const createEntry = createAsyncThunk(
  "topic/createEntry",
  async (payload: any) => {
    const { data: entryData, errors } = await altogic.endpoint.post(
      "/entry",
      payload
    );
