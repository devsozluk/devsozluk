    ...
    return (
        ...
        >
            {({ errors, values, setFieldValue, handleSubmit }) => (
                <TextArea value={values.content} errorMessage={errors.content} onChange={(event) => setFieldValue("content", event.target.value)} placeholder="Göndermek istediğin mesajı yaz." rows={4} >
                    <TextArea.Actions>
                        <Button loading={isLoading} onClick={() => handleSubmit()} className="ml-auto" size="sm">Gönder</Button>
                    </TextArea.Actions>
                </TextArea>
            )}
        </Formik>
